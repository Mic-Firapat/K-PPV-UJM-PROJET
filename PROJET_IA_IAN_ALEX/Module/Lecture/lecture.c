...
Point *chargement_fichier(char *nom_fichier, int *n, int *nbclasses){
  ...
  if (fscanf(fichier, "%d %d %d\n",n, &dimensions, nbclasses) != 3){
    fprintf(stderr,"Lecture de la première ligne (n dimensions nbclasses) ratée\n");
    fclose(fichier);
    return NULL;
  }
  ...
  for (i=0; i < *n; i++){
    if (fscanf(fichier, "%d %f %f\n", &(t[i].classe), &(t[i].x), &(t[i].y)) != 3){
      fprintf(stderr,"Lecture d'une ligne de données sur un point ratée");
      fclose(fichier);
      return NULL;
    }
    if(t[i].classe > CMAX){
      fprintf(stderr, "ERREUR lecture.c : Un des points à une classe dépassant le maximum reconnu (classe > %d)", CMAX);
      fclose(fichier);
      return NULL;
    }
  }
